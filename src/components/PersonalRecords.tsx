...
+import { Trophy, TrendingUp, Calendar, Wind } from 'lucide-react';
 import { PersonalRecord } from '../types';
-import { formatTime, events, eventCategories } from '../utils/trackData';
+import { formatTime, events, eventCategories, formatWindSpeed, isWindAffectedEvent } from '../utils/trackData';
...
+                  <div key={record.id} className={`bg-gradient-to-br from-gray-50 to-gray-100 rounded-lg p-4 border-l-4 ${
+                    record.isWindLegal === false ? 'border-orange-400' : 'border-yellow-400'
+                  }`}>
                     <div className="flex items-center justify-between mb-2">
                       <h4 className="font-semibold text-gray-900">{record.eventName}</h4>
-                      <Trophy className="w-5 h-5 text-yellow-500" />
+                      <div className="flex items-center space-x-1">
+                        {record.isWindLegal === false && (
+                          <Wind className="w-4 h-4 text-orange-500" title="Wind-assisted (not legal)" />
+                        )}
+                        <Trophy className={`w-5 h-5 ${record.isWindLegal === false ? 'text-orange-500' : 'text-yellow-500'}`} />
+                      </div>
                     </div>
...
+                        {record.windSpeed !== undefined && isWindAffectedEvent(record.eventType) && (
+                          <span className={`text-sm ml-2 ${record.isWindLegal === false ? 'text-orange-600' : 'text-gray-600'}`}>
...
+                          Wind-assisted (not legal for records)
+                        </div>
+                      )}
                     </div>
                   </div>
