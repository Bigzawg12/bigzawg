...
+import { events, eventCategories, parseTimeInput, trainingTemplates, isWindAffectedEvent } from '../utils/trackData';
         ...
         ? {
             ...
             sets: ex.sets.map(set => 
               set.id === setId 
                 ? { 
                     ...set, 
                     [field]: field === 'value' && typeof value === 'string' ? parseTimeInput(value) : 
                              field === 'windSpeed' ? parseFloat(value) || 0 : value
                   }
                 : set
             )
           }
...
+  const showWindSpeed = isWindAffectedEvent(eventType);
...
+                          step="0.1"
+                          value={set.windSpeed || ''}
+                          onChange={(e) => updateSet(exercise.id, set.id, 'windSpeed', e.target.value)}
+                          placeholder="0.0"
