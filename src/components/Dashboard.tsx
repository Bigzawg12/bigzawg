...
+import { Calendar, Clock, TrendingUp, Zap, Wind } from 'lucide-react';
 import { Workout, PersonalRecord, UserProfile } from '../types';
-import { formatTime, events } from '../utils/trackData';
+import { formatTime, events, formatWindSpeed, isWindAffectedEvent } from '../utils/trackData';
...
+                <div key={record.id} className={`flex items-center justify-between p-3 bg-gradient-to-r from-yellow-50 to-orange-50 rounded-lg border-l-4 ${
...
+                    <p className={`text-lg font-bold ${record.isWindLegal === false ? 'text-orange-700' : 'text-yellow-700'}`}>
...
+                    <p className={`text-xs ${record.isWindLegal === false ? 'text-orange-600' : 'text-yellow-600'}`}>
+                      NEW PR!
+                    </p>
                   </div>
                 </div>
