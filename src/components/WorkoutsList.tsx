...
+import { Calendar, Clock, Eye, Trash2, Wind } from 'lucide-react';
 import { Workout } from '../types';
-import { events, formatTime } from '../utils/trackData';
+import { events, formatTime, formatWindSpeed, isWindAffectedEvent } from '../utils/trackData';
...
+                          {set.unit === 's' ? formatTime(set.value) : `${set.value}${set.unit}`}
+                        </span>
+                        {set.windSpeed !== undefined && isWindAffectedEvent(workout.eventType) && (
+                          <div className="text-xs text-gray-500 flex items-center">
+                            <Wind className="w-3 h-3 mr-1" />
+                            {formatWindSpeed(set.windSpeed)}
+                          </div>
+                        )}
+                      </div>
                       {set.rest && (
